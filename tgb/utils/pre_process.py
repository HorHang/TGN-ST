from typing import Optional, cast, Union, List, overload, Literal, Dict, Any, Tuple
...
# def _get_sample_nodes(fname: str, k= 100) -> list:
#     df = pd.read_csv(fname)
#     sample_nodes = {node: 1 for node in np.random.choice(df.airport_code.to_numpy(), k)}
#     return sample_nodes
# Continent: AF, AN, AS, EU, NA, OC, SA
# 1. Random sample
# 2. Select based on Continent

# def _get_sample_nodes(fname: str, method= 'random', k= 100) -> dict:
#     df = pd.read_csv(fname)
#     if method == 'random':
#         df_sampl = df.sample(k)
#         df_sampl.to_csv(fname, na_rep= "NA", index= False)
#         return {node:1 for node in df_sampl.airport_code}
#     else: # continent
#         # assert method in ['AF', 'AN', 'AS', 'EU', 'NA', 'OC', 'SA']
#         # raise ValueError("method must be continent in 'AF', 'AN', 'AS', 'EU', 'NA', 'OC', 'SA'")
#         return {node: 1 for node in df.airport_code if df.continent == method}

def _get_sample_nodes(fname: str, method= 'random', deploy_region= None, k= 100) -> dict:
    assert method in ['random', 'AF', 'AN', 'AS', 'EU', 'NA', 'OC', 'SA'], "Invalid method. Must be 'random', 'AF', 'AN', 'AS', 'EU', 'NA', 'OC', 'SA'"
    sample_nodes = {}
    print(f'sub-sampling graph by {method} ...')
    
    if method == 'random':
        with open(fname, "r") as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=",")
            num_lines = sum(1 for _ in csv_reader) - 1
            rnd_l = {}
            while len(rnd_l) < k:
                rnd = random.randint(1, num_lines) # sampling all row skipping header
                if rnd not in rnd_l: # random sampling without repeat
                    rnd_l[rnd] = 1
        
        with open(fname, "r") as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=",")
            idx = 0
            for raw in tqdm(csv_reader):
    ...
    print(f'number of node count by {method} samples: {len(sample_nodes)}')
...
#############################################################################
# II. TODO Preprocess for data drift
# 1. get the target nodes for train/val and testing --> Done
# 2. Obtain time set for train/val and test
# 3. extract the edge of train/val within first 85% of train/val dataset based on the last train/val timestamp
# 4. extract the edge of test within the last 15% of the test dataset from train/val last timestamp
# 5. return df, edge_feat, node_feat condition on sample train/val (85%) & test(15%) nodes and timestamp
#############################################################################

################################### Graph Subsampling #######################
# I. Preprocess without data drift
# 1. Obtain the node list of the graph
# 2. Sampling k nodes
# 3. Obtain the new edgelist of the sampled nodes
# 4. Return pd, feat_l, node_ids based on new edge list
#############################################################################

    if sub_sampling:
        sample_nodes = _get_sample_nodes(node_feat, method= method, k= k)
        
        
        if drift:
            test_nodes = _get_sample_nodes(node_feat, method= "OC", k= k)
            ts_l = []
            
            with open(fname, "r") as csv_file:
                csv_reader = csv.reader(csv_file, delimiter=",")
                for row in tqdm(csv_reader):
                    ts, src, dst = row[0], row[1], row[2]
                    if (src in sample_nodes or dst in sample_nodes):
                        ts_l.append(int(ts))
            start_test_time = np.quantile(ts_l, 0.85)
            print(start_test_time)
            with open(fname, "r") as csv_file, open(fname, "r") as csv_file_test:
                csv_reader = csv.reader(csv_file, delimiter=",")
                csv_reader_test = csv.reader(csv_file_test, delimiter= ",")
                print("Processing edge sampling on whole temporal graph ...")
                num_lines = sum(1 for line in tqdm(csv_reader) if (line[1] in sample_nodes or line[2] in sample_nodes) and int(line[0]) <= start_test_time)
                test_lines = sum(1 for line in tqdm(csv_reader_test) if (line[1] in test_nodes or line[2] in test_nodes) and int(line[0]) > start_test_time)
                num_lines += test_lines
                print("number of edges counted", num_lines)
                u_list = np.zeros(num_lines)
                i_list = np.zeros(num_lines)
                ts_list = np.zeros(num_lines)
                label_list = np.zeros(num_lines)
            ...
            with open(fname, "r") as csv_file:
                ...
                for row in tqdm(csv_reader):
                    if idx == 0:
                        ...
                        src = row[1]
                        dst = row[2]
                        if  (int(ts) <= start_test_time and (src in sample_nodes or dst in sample_nodes)) or \
                            (int(ts) > start_test_time and (src in test_nodes or dst in test_nodes)):
                            ...
                            label_list[idx - 1] = (int(ts) > start_test_time) # check train/val or test
            ...
            with open(fname, "r") as csv_file:
                csv_reader = csv.reader(csv_file, delimiter=",")
                print("Processing edge sampling on whole temporal graph ...")
                num_lines = sum(1 for line in tqdm(csv_reader) if line[1] in sample_nodes or line[2] in sample_nodes)
                print("number of edges counted", num_lines)
                u_list = np.zeros(num_lines)
                i_list = np.zeros(num_lines)
                ts_list = np.zeros(num_lines)
...
def process_node_feat(
    ...
    """
    # feat_size = 20 ### Hang need to not hardcode the feature size
    ...
    with open(fname, "r") as csv_file:
        ...
        feat_size = type_idx + cont_idx + 7 + 2 # 7 is max len iso_region and 2 is len([lat, long])
        node_feat = np.zeros((len(node_ids), feat_size))
