...
class LinkPropPredDataset(object):
    def __init__(
        self,
        name: str,
        sub_sampling: bool = False, # Hang added
        method: str = 'random',
        drift: bool = False,
        ...
        if preprocess:
            self.pre_process(sub_sampling= sub_sampling, method = method, drift= drift)
    ...
    def generate_processed_files(self, sub_sampling= False, method = 'random', drift= False) -> pd.DataFrame:
        r"""
        ...
        if (osp.exists(OUT_DF)) and (self.version_passed is True):
            ...
            if self.name == "tgbl-flight":
                df, edge_feat, node_ids, start_test_time = csv_to_pd_data(self.meta_dict["fname"], node_feat= self.meta_dict["nodefile"], sub_sampling= sub_sampling, method = method, drift= drift)
                ### Hang modified
        ...
        return df, edge_feat, node_feat, start_test_time

    def pre_process(self, sub_sampling= False, method = 'random', drift = False): # Hang added sub_sampling
        """
        ...
        df, edge_feat, node_feat, start_test_time = self.generate_processed_files(sub_sampling= sub_sampling, method= method, drift= drift)
        ...
        _train_mask, _val_mask, _test_mask = self.generate_splits(full_data, start_test_time= start_test_time)
    ...
    def generate_splits(
        ...
        if start_test_time == 0:
            ...
            test_time = start_test_time
            train_val_ts = full_data["timestamps"][full_data["timestamps"] <= test_time]
            val_time = np.quantile(train_val_ts, 0.85)

        timestamps = full_data["timestamps"]
        train_mask = timestamps <= val_time
        val_mask = np.logical_and(timestamps <= test_time, timestamps > val_time)
        test_mask = timestamps > test_time
        print(f"validation point: {val_time}")
        print(f"Test point: {test_time}")
...
def main():
    name = "tgbl-flight" 
    dataset = LinkPropPredDataset(name=name, sub_sampling= True, root="datasets", preprocess=True)
    # Hang added sub_sampling
    dataset.node_feat
