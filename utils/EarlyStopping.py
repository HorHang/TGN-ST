...
class EarlyStopping(object):
    ...
    def step(self, metrics: list, model: nn.Module):
        ...
        for metric_tuple in metrics:
            ...
            if higher_better:
                if self.best_metrics.get(metric_name) is None or (metric_value - self.best_metrics.get(metric_name))/np.abs(self.best_metrics.get(metric_name)) > 1e-6:
        ...
        if torch.all(torch.tensor(metrics_compare_results[-1])):
            for metric_tuple in metrics:
