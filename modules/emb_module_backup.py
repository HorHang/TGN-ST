...
import torch
from modules.flash_attn_conv import TransformerFlashConv
...
class GraphAttentionEmbedding(torch.nn.Module):
    ...
    def forward(self, x, last_update, edge_index, t, msg):
        ...
        return self.conv(x, edge_index, edge_attr) # [num_nodes=unique node size, out_channels= 100]

class GraphFlashAttentionEmbedding(torch.nn.Module):
    """
    Reference:
    - https://github.com/pyg-team/pytorch_geometric/blob/master/examples/tgn.py
    """
    ...
    def __init__(self, in_channels, out_channels, msg_dim, time_enc):
        ...
        self.time_enc = time_enc
        edge_dim = msg_dim + time_enc.out_channels
        self.conv = TransformerFlashConv(
            in_channels, out_channels // 2, heads=2, dropout=0.1, edge_dim=edge_dim
        )

    def forward(self, x, last_update, edge_index, t, msg): # x is the node memory updated of the neighbors
        rel_t = last_update[edge_index[0]] - t
        rel_t_enc = self.time_enc(rel_t.to(x.dtype))
        edge_attr = torch.cat([rel_t_enc, msg], dim=-1)
