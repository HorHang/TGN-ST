...
class LastNeighborLoader:
    ...
    def __call__(self, n_id: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        # print("filtered neighbors:", neighbors.shape, neighbors)
        ...
        # print("relabel neighbors:", neighbors.shape, neighbors)
    ...
    def insert(self, src: Tensor, dst: Tensor):
        ...
        neighbors = torch.cat([src, dst], dim=0)    # batch_size * 2
        nodes = torch.cat([dst, src], dim=0)        # batch_size * 2
        e_id = torch.arange(                        # batch_size * 2
            self.cur_e_id, self.cur_e_id + src.size(0), device=src.device
        ).repeat(2)
        
        # print("neighbors:", neighbors.shape, neighbors)
        # print("nodes:", nodes.shape, nodes)
        # print("e_id:", e_id.shape, e_id)
        ...
        dense_id += self._assoc[nodes].mul_(self.size)  # batch_size * 2
        
        # print("dense_id:", dense_id.shape, dense_id)

        dense_e_id = e_id.new_full((n_id.numel() * self.size,), -1)
        dense_e_id[dense_id] = e_id
        dense_e_id = dense_e_id.view(-1, self.size)     # [unique nodes, size]
        
        # print("dense_e_id:", dense_e_id.shape, dense_e_id)

        dense_neighbors = e_id.new_empty(n_id.numel() * self.size)
        dense_neighbors[dense_id] = neighbors
        dense_neighbors = dense_neighbors.view(-1, self.size)   # [unique nodes, size]
        
        # print("dense_neighbors:", dense_neighbors.shape, dense_neighbors)
        ...
        # print("final e_id:", e_id.shape, e_id)
        # print("final neighbors:", neighbors.shape, neighbors)
