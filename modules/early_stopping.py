...
class EarlyStopMonitorChkpt(object):
    ...
    def get_latest_model_path(self):
        ...
        """
        return self.save_model_dir + '/{}_latest.pth'.format(self.save_model_id)
    
    def step_check(self, curr_metric: float, models_dict: dict, optimizer):
        r"""
        execute the early stop strategy
        :param: metric: a metric to evaluate the early stopping on
        :param: models_dict: a dictionary containing all models to be saved
        ...
        self.save_checkpoint(models_dict, optimizer, self.get_latest_model_path())
        
        if (self.best_sofar is None) or ((curr_metric - self.best_sofar) / np.abs(self.best_sofar) > self.tolerance):
            # first iteration or observing an improvement
            self.best_sofar = curr_metric
            print("INFO: save a checkpoint...")
            self.save_checkpoint(models_dict, optimizer, self.get_best_model_path())
            self.counter = 0
            self.best_epoch = self.epoch_idx
        else:
            # no improvement observed
    ...
    def save_checkpoint(self, models_dict: dict, optimizer, model_path):
        r"""
        save models as a checkpoint
        :param: models_dict: a dictionary containing all models to be saved 
        """
        ...
        model_names = list(models_dict.keys())
        model_components = list(models_dict.values())
        checkpoint_dict = {model_names[i]: model_components[i].state_dict() for i in range(len(model_names))}
        checkpoint_dict['optimizer'] = optimizer.state_dict()
        checkpoint_dict['epoch'] = self.epoch_idx
        checkpoint_dict['best_epoch'] = self.best_epoch
        checkpoint_dict['counter'] = self.counter
        checkpoint_dict['best_sofar'] = self.best_sofar
        torch.save(checkpoint_dict, model_path)

    def load_checkpoint(self, models_dict: dict, state: str):
        r"""
        save models from the checkpoint
        :param: models_dict: a dictionary containing all models
        """
        if state == 'best':
            ...
            model_latest_path = self.get_latest_model_path()
            checkpoint = torch.load(model_latest_path)
            print("INFO: load the latest saved model state from {}".format(model_latest_path))
            for model_name, model in models_dict.items():
                model.load_state_dict(checkpoint[model_name])
            self.epoch_idx = checkpoint['epoch']
            self.counter = checkpoint['counter']
            self.best_sofar = checkpoint['best_sofar']