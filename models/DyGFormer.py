...
from models.module import TimeEncoder, TimeEncoderMixer
...
class DyGFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.time_encoder = TimeEncoderMixer(time_dim=time_feat_dim)
    ...
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
                     padded_nodes_neighbor_times: np.ndarray, time_encoder: TimeEncoderMixer):
        """
...
class NeighborCooccurrenceEncoder(nn.Module):
    ...
    def __init__(self, neighbor_co_occurrence_feat_dim: int, device: str = 'cpu'):
        ...
        self.neighbor_co_occurrence_feat_dim = neighbor_co_occurrence_feat_dim # dim 50
        self.device = device
    
        self.neighbor_co_occurrence_encode_layer = nn.Sequential(
            nn.Linear(in_features=1, out_features=self.neighbor_co_occurrence_feat_dim), # (1,50)
            nn.ReLU(),
            nn.Linear(in_features=self.neighbor_co_occurrence_feat_dim, out_features=self.neighbor_co_occurrence_feat_dim)) # (50,50)
    ...
    def count_nodes_appearances(self, src_padded_nodes_neighbor_ids: np.ndarray, dst_padded_nodes_neighbor_ids: np.ndarray):
       ...
       # src_padded_node_neighbor_ids, ndarray, shape (src_max_seq_length, )
        ...
        # Tensor, shape (batch_size, src_max_seq_length, 2) ; ex. [[2,1], [1,2], [2,1]]^T and [[1,2], [1,2], [2,1], [0,1]^T
