...
class TimeEncoderMixer(torch.nn.Module):
    ...
    def __init__(self, time_dim):
        ...
        self.time_dim = time_dim
        self.lin = Linear(1, time_dim)
        self.reset_parameters()
    
    def reset_parameters(self, ):
        self.lin.weight = Parameter((torch.from_numpy(1 / 10 ** np.linspace(0, 9, self.time_dim, dtype=np.float32))).reshape(self.time_dim, -1))
        self.lin.weight.requires_grad = False
    
    @torch.no_grad()
    def forward(self, timestamps):
        timestamps = timestamps.unsqueeze(dim=2)
        return self.lin(timestamps).cos()
...
class KanMergeLayer(nn.Module):
    ...
    def __init__(self, input_dim1: int, input_dim2: int, hidden_dim: int, output_dim: int):
        ...
        """
        super().__init__()
        self.fc1 = KANLinear(input_dim1 + input_dim2, hidden_dim)
        self.fc2 = KANLinear(hidden_dim, output_dim)
        # self.act = nn.ReLU()
