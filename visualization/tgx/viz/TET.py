...
def TET(temp_edgelist : Union[object, dict],
    ...
    plot_edge_presence_matrix(e_presence_mat, date_ls, test_split_ts_value, unique_ts_list, list(idx_edge_map.keys()),
                              fig_param, test_split = test_split, add_frames=add_frame, show=show, ticks_font_size = ticks_font_size, title=title)
    return 
...
def plot_edge_presence_matrix(e_presence_mat,
    ...
    plt.title(title, fontsize=axis_title_font_size)
    ...
    if date_ls is not None:
        ax.set_yticks(range(len(date_ls)))
        ax.set_yticklabels(date_ls[::-1], rotation=0, fontsize=ticks_font_size)
        locator = matplotlib.ticker.MaxNLocator(nbins=7)
        ax.yaxis.set_major_locator(locator)

    labels = ["$E_{train} \setminus E_{test}$", "$E_{train} \cap E_{test}$", "$E_{test} - E_{train}$"]
    new_colors = ['#018571',  # E_ONLY_TRAIN    2c7bb6
                  '#fc8d59',  # E_TRAIN_AND_TEST
                  '#b2182b'  # E_INDUCTIVE
                ]
    
    legend_handles = [mpatches.Patch(color=color, label=label) for color, label in zip(new_colors, labels)]
    plt.legend(handles= legend_handles, title = "Edge Type", loc= "lower right")
    plt.show()
