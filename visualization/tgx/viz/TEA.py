...
def TEA_plot_edges_bar(ts_edges_dist: list,
    ...
    # plotting stuffs
    # bar plot
    plt.bar(timestamps, repeated, label='$Edge_{reoccurance}$', color='#404040', alpha=0.4)
    plt.bar(timestamps, new, label='$Edge_{new}$', bottom=repeated, color='#ca0020', alpha=0.8, hatch='//')
    # test split line
    if test_split:
    ...
    if date_ls is not None:
        plt.xticks(range(0, len(date_ls)), date_ls, rotation=70, fontsize=ticks_font_size)
